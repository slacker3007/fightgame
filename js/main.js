...
canvas.addEventListener('mousedown', e => {
    ...
    if (clickedBtn) { clickedBtn.action(); return; }

    if (state === "menu" && userName.length > 0) { initPlayer(); startLevel(1); }
    else if (state === "inventory") {
        if (selectedInvItem && mx > 875 && mx < 905 && my > 115 && my < 145) { selectedInvItem = null; return; }
...
function updateUIButtons() {
    uiButtons = [];
    if (state === "camp") {
        createButton(450, 380, 180, 50, "camp", "INVENTORY", COLORS.GRAY, () => state = "inventory");
        createButton(650, 380, 180, 50, "camp", "NEXT FIGHT", COLORS.BTN_BLUE, () => { currentLvl++; startLevel(currentLvl); });
    }
    if (state === "inventory") {
        createButton(430, 530, 150, 40, "inventory", "BACK", COLORS.GRAY, () => { state = "camp"; selectedInvItem = null; });
        if (selectedInvItem) {
    ...
    if (state === "gameover" || state === "victory") {
        createButton(380, 480, 200, 60, state, "NEW JOURNEY", COLORS.BTN_BLUE, () => { state = "menu"; userName = ""; score = 0; currentLvl = 1; });
    }
}

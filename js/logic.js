 ...
 * Total functionality preserved: Stat calculation, Turn Resolution, 
 * Leveling, and Logging systems.
 */

function initPlayer() {
    // Initializes player with base stats of 5 and empty equipment.
    player = { 
...
function calcStats() {
    // Resets bonuses and iterates through equipped items to apply stat modifiers.
    ...
    // Derived stats: HP scales with STA, DMG with STR, Dodge with DEX, and Crit with LUCK.
...
function startLevel(lvl) {
    // Pulls enemy data based on current level index.
...
async function resolveTurn() {
    ...
    // --- PLAYER ATTACK PHASE ---
    // Enemy randomly shields 2 out of 5 zones.
    ...
    if(eBlkArr.includes(selAtk)) {
        ...
        // Logic check: Headshot (Zone 1) provides 1.4x multiplier.
    ...
    // Delay to allow player to see the results of their move.
    ...
    // Reset selection and release processing lock.
    selAtk = null; 
    selBlk = []; 
    isProcessing = false;
    checkEnd();
...
function checkEnd() {
    // Handles victory, item drops, and game over states.
    if(enemy.hp <= 0) {
        ...
        if(currentLvl === 10) { 
            ...
            // Pulls a random item from ALL_ITEMS as a reward.
...
function spawnText(txt, x, y, col) { 
    particles.push({ txt, x, y, col, life: 1.0, vy: -2 }); 
}

function saveScore() {
    // Saves top 5 high scores to local storage.
